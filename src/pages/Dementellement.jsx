...
export default function Demmentellement(props) {
    ...
    const columns = [
        ...
        },
        {
            field: 'dc',
            headerName: 'DC',
            description: 'signification de dc',
            type: 'decimal',
            width: 130,
        },
        {
            field: 'vr',
            headerName: 'VR',
            description: 'signification de vr',
            type: 'decimal',
            width: 130,
        },
        {
            field: 'dctg',
            headerName: 'DCtg',
            description: 'signification de DCtg',
            width: 90,
            valueGetter: (params) => `${(Actu((params.row.dc - params.row.vr) * params.row.quantite, props.actualisation, props.actualisationReel / 100, props.persistance) / (props.persistance))}`,
    ...
    React.useEffect(() => {
        var cmt = Actu((item.dc - item.vr) * item.quantite, props.actualisation, props.actualisationReel / 100, props.persistance) / (props.persistance)
        setitem({ ...item, dctg: cmt })
        console.log('that is item :)(: 1', item)
    }, [item.dc, item.vr, item.quantite, props.actualisation, props.actualisationReel, props.persistance])


    React.useEffect(() => {
        props.setexternalitesDementellement(externalites)
    }, [externalites])
    ...
    return (
        <div >
            ...
            <Dialog open={openDiag} onClose={handleClose}>
                <DialogTitle>Démentellement</DialogTitle>
                <DialogContent>
                    ...
                    <form onSubmit={(event) => {
                        ...
                        <TextField
                            ...
                            margin="dense"
                            value={item.dc}
                            onChange={(e) => setitem({ ...item, dc: e.target.value })}
                            required
                            id="dc"
                            label="dc"
                            type="decimal"
                            helperText='signification de dc'
                            fullWidth
                            variant="standard"
                        />
                        ...
                        <TextField
                            ...
                            margin="dense"
                            value={item.vr}
                            onChange={(e) => setitem({ ...item, vr: e.target.value })}
                            required
                            id="vr"
                            label="vr"
                            type="decimal"
                            helperText='signification de vr'
                            fullWidth
                            variant="standard"
                        />
